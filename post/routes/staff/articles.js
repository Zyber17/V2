...
(function() {
  var db, es, findArticle, findSection, htmlToText, marked, moment, photo_bucket_name, string;
                ...
                if (resp.status === 4 && moment().toDate() > resp.publishDate) {
                  es.index({
                    ...
                    body: {
                      ...
                      slug: resp.slug,
                      photo: resp.photos[0] ? (resp.photos.length > 1 ? resp.photos[resp.photos.length - 2].name : resp.photos[0].name) : void 0,
                      body: resp.bodyPlain,
  ...
  exports.edit_get = function(req, res, next) {
    return db.Sections.find().select({
      ...
      if (req.session.message) {
        ...
        return res.render('staff/edit', req.session.message);
      } else {
        return findArticle(req.params.slug, false, function(err, article) {
          ...
          if (!err) {
            if (article) {
              ...
              return res.render('staff/edit', content);
            } else {
              return res.render('errors/404', {
                err: "Article not found"
              });
  ...
  exports.edit_post = function(req, res, next) {
    ...
    if (err.length > 0) {
      ...
      return res.redirect("/staff/articles/" + req.params.slug + "/edit");
    } else {
      return findArticle(req.params.slug, false, function(err, resp) {
        if (!err) {
          if (resp) {
            return findSection(req.body.section, function(err, section_resp) {
              var oldPublishdate, oldStatus;
              if (!err) {
                if (section_resp) {
                  ...
                  return resp.save(function(err, resp) {
                    if ((resp.status === 4 && moment().toDate() > resp.publishDate) && (oldStatus !== 4 || moment().toDate() < oldPublishdate || !oldPublishdate)) {
                      ...
                      return res.end("(Articles ~400): This should never happen");
                    }
                  });
                } else {
                  return res.render('errors/404', {
      ...
      return db.Articles.findOne({
        ...
        if (!err) {
          return es["delete"]({
            ...
            if (!err) {
              return resp.remove(function(err, resp) {
                if (!err) {
